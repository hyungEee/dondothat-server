...
입출금 계좌 데이터 연동 시 카드와 달리 업장 카테고리 정보가 제공되지 않아 자체 카테고리 분류 로직을 설계. </br>
주요 키워드(ex.프랜차이즈 상호명)로 1차 필터링을 수행하고, 남은 항목만 LLM이 판단하는 하이브리드 방식을 고안. </br></br>
...
한번에 많은 양의 데이터 입력 시에는 비동기 호출을 적용하여 응답 속도를 개선. </br>
배치와 세마포어 사이즈에 따른 테스트를 진행하여 속도와 비용 면에서 최적의 값을 적용, 평균 응답 속도를 약 65% 단축. </br>
출력형식 강제, 응답 유효성 검사 및 비정상 출력시 기본값 폴백을 적용하여 안정성을 보장.</br></br>
